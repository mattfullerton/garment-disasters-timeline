...
var sql_simple = new cartodb.SQL({
  user: 'crowdsalad',
  format: 'json'
});
/* DB connection - geo */
...
/* Use the minimum and maximum year to define the bounds of the data */
sql_simple.execute(query).done(function (response) {
  ...
  /* Store lowest year */
  firstYear = response.rows[0].year;
  /* Fill the correct elements with the correct data */
...
function updateLayers(event) {
  var year = years[0] + $("div#slider").slider("value");
  updateNumbers(year);
  $.each(responses, function (index, obj) {
    if ((obj.features.length > 0) && (obj.features[0].properties.year <= year)) {
      $.each(obj.features, function (index, feature) {
        map.addLayer(feature.properties.theLayer);
      });
    }
  });
  ...
  //for (var i = layerno + 1; i < years.length; i++) {
    $.each(responses, function (index, obj) {
      if ((obj.features.length > 0) && (obj.features[0].properties.year > year)) {
        $.each(obj.features, function (index, feature) {
          map.removeLayer(feature.properties.theLayer);
        });
      }
    });
  //}
...
function updateNumbers(year) {
  ...
  if (years.length > 0) {
    for (var i = 0; i < years.length; i++) {
      data["y" + years[i]] = killed_values[year - years[0]][i];
    }
  }
  data.id = "v1";
...
function init() {
  ...
  for (var i = years[0]; i <= years[years.length - 1]; i++) {
    var query =
      "select the_geom, no_killed_numeric, location, EXTRACT(YEAR FROM date) as year, date, date_only_year, factory_incident_company_name, source_not_exhaustive_need_to_list_all_pdfs, supplying, text_description, type_of_incident from bangladesh_clothing_diasters_for_cartadb WHERE date > '" +
